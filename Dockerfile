...
RUN apt-get update \
    ...
    && addgroup --system dpnai \
    && adduser --system --ingroup dpnai --home /opt/dpn-ai --no-create-home dpnai
...
COPY . .
RUN mkdir -p data workspace/generated workspace/uploads plugins skills runtime_logs install_logs \
    && chown -R dpnai:dpnai /opt/dpn-ai
