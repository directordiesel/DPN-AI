...
def test_file_console_is_internal_and_pairing_gated():
    ...
    assert "fileButton.isEnabled = enabled" in main
    assert "Intent(this@MainActivity, FileActivity::class.java)" in main
...
def test_general_upload_reuses_authenticated_https_api_and_blocks_executable_packages():
    ...
    assert 'baseUri.scheme.equals("https", ignoreCase = true)' in client
...
def test_file_analysis_uses_unified_chat_attachment_path():
    ...
    assert 'require(!clean.startsWith("/") && !clean.contains(".."))' in client
