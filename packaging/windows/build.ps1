...
$UpdateTrustPublicKeySha256 = $null
if ($UpdateTrustConfigured) {
    ...
    $UpdateTrustData = Get-Content $UpdateTrustFile -Raw | ConvertFrom-Json
    if ($UpdateTrustData.repository -ne "directordiesel/DPN-AI") {
        throw "Update trust root is not bound to the official DPN-AI repository."
    }
    if ([int]$UpdateTrustData.schema_version -ne 1) {
        throw "Update trust root schema version is invalid."
    }
    if ([string]$UpdateTrustData.key_id -ne "release-ed25519-v1") {
        throw "Update trust root key id is not authorized."
    }
    $UpdateTrustPublicKeyHex = ([string]$UpdateTrustData.ed25519_public_key_hex).ToLowerInvariant()
    if ($UpdateTrustPublicKeyHex -notmatch '^[0-9a-f]{64}$') {
