    ...
    def restore(self, snapshot_id: str, overwrite: bool = False) -> dict[str, Any]:
        ...
        try:
            with zipfile.ZipFile(archive, "r") as zf:
                ...
                if not valid:
                    self.db.audit("snapshot.restore_rejected", f"Rejected snapshot restore {snapshot['name']}", {"snapshot_id": snapshot_id, "reason": error})
            ...
            self.db.audit("snapshot.restore_rejected", f"Rejected snapshot restore {snapshot['name']}", {"snapshot_id": snapshot_id, "reason": "invalid archive"})
